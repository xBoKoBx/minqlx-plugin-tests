...
class mydiscordbot(Plugin):
    ...
    * qlx_discord_extensions (default: "") discord extensions to load after initializing
    ...
    def game_status_information(game):
        ...
        return (
            f"{ginfo} on **{Plugin.clean_text(maptitle)}** ({gametype}) with **{num_players}/{max_players}** players. "
        )
    ...
    def handle_vote_started(self, caller, vote, args):
        ...
        content = f"_{caller_name} called a vote: {vote} {discord.utils.escape_markdown(Plugin.clean_text(args))}_"
    ...
    def handle_vote_ended(self, votes, _vote, _args, passed):
        ...
        """
        content = f"*Vote passed ({votes[0]} - {votes[1]}).*" if passed else "*Vote failed.*"
...
class SimpleAsyncDiscord(threading.Thread):
    ...
    def relay_chat_message(self, player, channel, message):
        ...
        content = (
            f"**{discord.utils.escape_markdown(player.clean_name)}**{channel}: {discord.utils.escape_markdown(message)}"
        )
    ...
    def triggered_message(self, player, message):
        ...
        triggered_channels = (
            []
            if self.discord is None
            else [self.discord.get_channel(channel_id) for channel_id in self.discord_triggered_channel_ids]
        )
        channel_names = [f"#{channel.name}" for channel in triggered_channels if channel is not None]

        formatted_channel_names = "^7, ^5".join(channel_names)
        if len(channel_names) == 1:
            Plugin.msg(f"Message ^1{message}^7 sent to discord channel ^5{formatted_channel_names}^7!")
        else:
            Plugin.msg(f"Message ^1{message}^7 sent to discord channels ^5{formatted_channel_names}^7!")
